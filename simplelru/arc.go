...
// ARCLRU is a thread-safe fixed size Adaptive Replacement Cache LRU (ARC).
...
func NewARC(size int) (*ARCLRU, error) {
	...
	b2, err := NewLRU(size, nil)
	if err != nil {
		return nil, err
	}
	t1, err := NewLRU(size, nil)
	if err != nil {
		return nil, err
	}
	t2, err := NewLRU(size, nil)
	if err != nil {
		return nil, err
	}
...
func (c *ARCLRU) Get(key interface{}) (value interface{}, ok bool) {
...
func (c *ARCLRU) replace(b2ContainsKey bool) (k, v interface{}, ok bool) {
	t1Len := c.t1.Len()
...
func (c *ARCLRU) Len() int {
	return c.t1.Len() + c.t2.Len()
}

// Keys returns all the cached keys
func (c *ARCLRU) Keys() []interface{} {
	k1 := c.t1.Keys()
	k2 := c.t2.Keys()
	return append(k1, k2...)
}
...
func (c *ARCLRU) Contains(key interface{}) bool {
	return c.t1.Contains(key) || c.t2.Contains(key)
}

// Peek is used to inspect the cache value of a key
// without updating recency or frequency.
func (c *ARCLRU) Peek(key interface{}) (value interface{}, ok bool) {
	if val, ok := c.t1.Peek(key); ok {
		return val, ok
	}
